...
def main() -> int:
    ...
    if paths["db"].exists() and not args.rebuild_db:
        raise SystemExit(f"{paths['db'].relative_to(G.ROOT)} exists. Re-run with --rebuild-db to regenerate it from the intake.")
    master_wb = G.ROOT / "outputs" / "master" / "Silverleaf Master Database - Consolidated.xlsx"
    ...
    steps = [
        ("build run", [here / "build_government_run.py", *run]),
        ("export intake", [here / "export_government_run.py", *run]),
    ...
    for label, command in steps:
        print(f"\n== {label} ==", flush=True)
        result = subprocess.run([sys.executable, *map(str, command)], cwd=G.ROOT, capture_output=True, text=True, encoding="utf-8",
    ...
    print(f"\nDone. Workbook: {paths['workbook'].relative_to(G.ROOT)}")
    return 0
