...
| A third record with a shared name finds the wrong kind | Neema Village (home) and Neema Village Tanzania Inc (funder) share a name key; a third funder record could not find its funder twin and became a duplicate row | `consolidate_research.py` keeps one merge slot per name and per domain for funders and one for institutions |
| Supplementary contact records override research | A contact-profile record (slice W) with more sources became the merge base and replaced researched addresses and risk labels | Slice W sorts last in every merge: it fills blanks only; the export leaves out websites another run record uses |
| A website's person email looks like a general inbox | vd@vinnie.co.nz (a founder's initials at a vanity domain) was read as the home's inbox and collided with a risky contact | `build_contact_profiles.py` types initials and name-domain mailboxes as named, never as the organisation's route |
