...
- The consolidated workbook shows these drafts in Messages, Outreach plans and Sequences. Rebuild it with `npm run build:workbook` after any change. `outputs/messages/Silverleaf Offer-Aligned Messages - <date>.xlsx` shows every draft in all three databases.
- The design-only send steps point at these drafts: F01 steps 1–3, F03 step 1 and touchpoints C01 and C02. Their configuration is `automation-2026-09-23-offer-v4`, still disabled. The earlier text naming the v3 copy columns stays in `automation_recipe_history`. The v3 columns still hold the earlier copy, so never send from them.
- Re-running `draft_master_messages.py` changes nothing once the drafts are current. Run it again after a merge and `refresh_acquisition_metadata.py`, so each plan's copy matches its track (routing copy for AQ01).
