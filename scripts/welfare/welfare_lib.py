...
def _repo_root() -> Path:
    """Find the repository root by walking up to AGENTS.md, so this file works from scripts/welfare/ and from the skill's copy."""
...
ROOT = _repo_root()
SCRIPTS = Path(__file__).resolve().parent  # this copy's folder; scripts/welfare/ is the canonical, discoverable home
CANONICAL_SCRIPTS = ROOT / "scripts" / "welfare"
SKILL_SCRIPTS = ROOT / "skills" / "silverleaf-welfare-leads" / "scripts"  # identical bundled copy for standalone skill installs
SKILL = ROOT / "skills" / "silverleaf-welfare-leads"  # the skill that documents them (brief, slices, contract)
