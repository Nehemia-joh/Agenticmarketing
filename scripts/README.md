...
| Government leads: collect | `scripts/government/collect_ward_locations.py` | Wikipedia ward points, OpenStreetMap places, district boundaries, and schools and health facilities named after wards (for reviewed overrides) for placing wards | Wikipedia 2 s apart; Overpass one query at a time, 5 s apart, mirror fallback |
...
| Messages: hooks | `scripts/messaging/plan_hook_research.py` | Plans a research wave for first-message hooks and lead briefs: groups a track's plans by organisation and writes one slice and agent prompt per agent under `runtime/hooks/` | At most four agents; the search budget split evenly after a 10% reserve |
| Messages: hooks | `scripts/messaging/apply_hook_research.py` | Merges the wave into the master: lead briefs (facts with source links and verbatim excerpts), verified hooks and role checks; preview by default, `--apply` for one transaction | None; one transaction |
| Research: read a page | `scripts/contacts/read_page.py` | Prints one page's visible text for a research agent to quote exactly, with its status, robots.txt state and SHA-256 | Cached; 1.5 s per site; robots.txt Disallow honoured; blocks reported |
...
| Contacts: collect | `scripts/contacts/crawl_org_websites.py` | Crawls each organisation's own website (master and welfare; `.go.tz` excluded) for published emails, phones, postal addresses, official social pages and named people with roles (in a 'Name / Role' layout or a sentence such as 'founded in 2010 by …'); resumable; `--reextract` re-reads the cached pages with the current rules, and `--recrawl <domain>` crawls chosen sites again | robots.txt Disallow honoured (RFC 9309 matching); a site whose robots.txt cannot be read is crawled and flagged; one request per site at a time, 1.5 s apart; cached in `runtime/contacts/http-cache/`, so a re-run re-extracts without the network |
...
| Contacts: plan | `scripts/contacts/plan_contact_research.py` | Picks organisations no agent has searched that still lack an email or phone (`--target routes`), a named decision-maker (`decision-makers`: organisations with a route whose own site the crawler could not read) or either (`all`); ranks them nearest first, caps each slice (`--max-per-slice`) or, with `--split`, divides a larger slice into parts with an agent each, splits the wave's search budget over at most four slices and writes each agent's prompt | None; plans only. Launch at most four agents per wave within the session's WebSearch cap |
...
| Contacts: browser plan | `scripts/contacts/plan_browser_pass.py` | Plans a browser pass over own websites the crawler could not use: sites whose robots.txt disallows crawling (read at the user's direction; everything tagged risky) and readable pages built by script that gave nothing. Skips organisations already read, sorts by what each lacks, splits into slices and writes each browser agent's prompt | None; plans only. Browser agents use no web search, work in their own tab, never pass a bot check or login, and count toward four research agents at a time |
| Contacts: build | `scripts/contacts/build_contact_profiles.py` | One contact profile per organisation in all three databases, plus contact leads: typed emails and phones (a labelled hotline or a shared platform's inbox is never a route), sources, named people filtered to current, relevant roles, one lead per person from the search agent's record when there is one (every lead kept: agent-confirmed first, then by most senior role; a name given only in part is kept and labelled incomplete) | None |
